...
이름(job_title)과 최고월급(max_salary)을 최고월급의(max_salary) 내림차순(DESC)로 정렬하여 출력하세요.    
*/
select  job_title 업무이름,
		max_salary 최고월급
from jobs
...
이름(first_name), 월급, 커미션퍼센트 를  월급순(내림차순) 출력하세오. 단 커미션퍼센트 가 null 이면 0 으로 나타내시오.
*/
select  first_name 이름,
		salary 월급,
...
select  department_name 부서이름,
		character_length(department_name)
from departments
...
올림차순(ASC)으로 정렬해 보세오.
*/
select ucase(country_name) 나라이름
from countries
